...
mort = data.frame(matrix(NA, 6, 2))
colnames(mort) = c("Choke Score", "Projected Future ACL Attainment")
...
mort_tbl <- mort |>
  gt::gt() |>
  gt::tab_header(
    title = "Table 4. Choke scores applied based the percent of the projected ACL attainment given current average coastwide catches by species."
  ) |>
  gt::tab_options(
    table.font.size = 12
  ) |>
...
Additionally, in 2026, the choke score was set at 2 for shortspine thornyhead and yellowtail rockfish based upon comments from the Groundfish Advisory Panel and the Pacific Fishery Management Council regarding the potential for these species to be choke stocks given the expected ACLs in 2027-28 and within sector constraints in the groundfish fishery which are not reflected in the projected attainment percentages.