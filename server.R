...
shinyServer(function(input, output, session) {
  ...
  species_groups <- reactive({
    req(input$yr)
    species_groups <- readr::read_csv(
      here::here("tables", input$yr, "species_management_groups.csv"),
  ...
  output$management_groups1 <- renderUI({
    if (!is.null(species_groups)) {
      management_groups1 <- unique(as.character(
        species_groups()$`Management Group`
      ))
      selectInput(
  ...
  output$management_groups2 <- renderUI({
    if (!is.null(species_groups)) {
      management_groups2 <- unique(as.character(
        species_groups()$`Management Group`
      ))
      selectInput(
  ...
  output$management_groups3 <- renderUI({
    if (!is.null(species_groups)) {
      management_groups3 <- unique(as.character(
        species_groups()$`Management Group`
      ))
      selectInput(
  ...
  output$management_groups4 <- renderUI({
    if (!is.null(species_groups)) {
      management_groups4 <- unique(as.character(
        species_groups()$`Management Group`
      ))
      selectInput(
  ...
  output$management_groups5 <- renderUI({
    if (!is.null(species_groups)) {
      management_groups5 <- unique(as.character(
        species_groups()$`Management Group`
      ))
      selectInput(
  ...
  output$management_groups6 <- renderUI({
    if (!is.null(species_groups)) {
      management_groups6 <- unique(as.character(
        species_groups()$`Management Group`
      ))
      selectInput(
  ...
  output$management_groups7 <- renderUI({
    if (!is.null(species_groups)) {
      management_groups7 <- unique(as.character(
        species_groups()$`Management Group`
      ))
      selectInput(
  ...
  output$management_groups8 <- renderUI({
    if (!is.null(species_groups)) {
      management_groups8 <- unique(as.character(
        species_groups()$`Management Group`
      ))
      selectInput(
  ...
  output$management_groups9 <- renderUI({
    if (!is.null(species_groups)) {
      management_groups9 <- unique(as.character(
        species_groups()$`Management Group`
      ))
      selectInput(
  ...
  output$management_groups10 <- renderUI({
    if (!is.null(species_groups)) {
      management_groups10 <- unique(as.character(
        species_groups()$`Management Group`
      ))
      selectInput(
  ...
  fish_mort_data <- reactive({
    req(input$yr)
    fish_mort_data <- readr::read_csv(
      here::here("tables", input$yr, "1_fishing_mortality.csv"),
      show_col_types = FALSE
    )
    fish_mort_data
  })
  com_rev_data <- reactive({
    req(input$yr)
    com_rev_data <- readr::read_csv(
      here::here("tables", input$yr, "2_commercial_revenue.csv"),
      show_col_types = FALSE
    )
    com_rev_data
  })
  tribal_data <- reactive({
    req(input$yr)
    tribal_data <- readr::read_csv(
      here::here("tables", input$yr, "3_tribal_revenue.csv"),
      show_col_types = FALSE
    )
    tribal_data
  })
  rec_data <- reactive({
    req(input$yr)
    rec_data <- readr::read_csv(
      here::here("tables", input$yr, "4_recreational_importance.csv"),
      show_col_types = FALSE
    )
    rec_data
  })
  eco_data <- reactive({
    ...
    eco_data <- readr::read_csv(
      here::here("tables", input$yr, "5_ecosystem.csv"),
      show_col_types = FALSE
    )
    eco_data
  })
  stock_stat_data <- reactive({
    req(input$yr)
    stock_stat_data <- readr::read_csv(
      here::here("tables", input$yr, "6_stock_status.csv"),
      show_col_types = FALSE
    )
    stock_stat_data
  })
  assess_freq_data <- reactive({
    req(input$yr)
    assess_freq_data <- readr::read_csv(
      here::here("tables", input$yr, "7_assessment_frequency.csv"),
      show_col_types = FALSE
    )
    assess_freq_data
  })
  const_dem_data <- reactive({
    req(input$yr)
    const_dem_data <- readr::read_csv(
      here::here("tables", input$yr, "8_constituent_demand.csv"),
      show_col_types = FALSE
    )
    const_dem_data
  })
  new_info_data <- reactive({
    req(input$yr)
    new_info_data <- readr::read_csv(
      here::here("tables", input$yr, "9_new_information.csv"),
      show_col_types = FALSE
    )
    new_info_data
  })
  rebuilding_data <- reactive({
    req(input$yr)
    rebuilding_data <- readr::read_csv(
      here::here("tables", input$yr, "10_rebuilding.csv"),
      show_col_types = FALSE
    )
    rebuilding_data
  })
  ...
  output$warning <- renderText({
    if (sum_weights() != 1.00) {
      paste(
        "<span style=\"color:red\">WARNING: Ensure all weights add up to 1.</span>"
      )
    }
  })
  ...
  rescale_weights <- function(factor_weights) {
    ...
    # avoid zero division
    if (count == 0) {
      rescaled_weights <- rep(
        1 / length(factor_weights),
        length(factor_weights)
      )
    } else {
      rescaled_weights <- factor_weights
      rem <- (1.000 - sum_weights()) / count
      rescaled_weights[factor_weights > 0] <- factor_weights[
        factor_weights > 0
      ] +
        rem
    }
  ...
  observeEvent(
    input$rescale,
    {
      ...
      updateNumericInput(
        session,
        "comm_weight",
        value = round(factor_weights()[1], 4)
      )
      updateNumericInput(
        session,
        "rec_weight",
        value = round(factor_weights()[2], 4)
      )
      updateNumericInput(
        session,
        "tribal_weight",
        value = round(factor_weights()[3], 4)
      )
      updateNumericInput(
        session,
        "cd_weight",
        value = round(factor_weights()[4], 4)
      )
      updateNumericInput(
        session,
        "reb_weight",
        value = round(factor_weights()[5], 4)
      )
      updateNumericInput(
        session,
        "ss_weight",
        value = round(factor_weights()[6], 4)
      )
      updateNumericInput(
        session,
        "fm_weight",
        value = round(factor_weights()[7], 4)
      )
      updateNumericInput(
        session,
        "eco_weight",
        value = round(factor_weights()[8], 4)
      )
      updateNumericInput(
        session,
        "ni_weight",
        value = round(factor_weights()[9], 4)
      )
      updateNumericInput(
        session,
        "af_weight",
        value = round(factor_weights()[10], 4)
      )
    },
    ignoreInit = TRUE
  )
  ...
  output$overall_ranking <- renderPlotly({
    ...
    if (sum_weights() == 1.00) {
      ...
      overall_plot <- ggplot(
        top_species,
        aes(
          x = reorder(rank_species, score, sum),
          y = score,
          fill = factor,
          text = paste0(
            "Species: ",
            ...
            paste0(
              ...
              " (",
              round((score / `Weighted Total Score`) * 100, digits = 1),
              "%)"
            ),
        ...
        labs(
          ...
          y = "Overall Weighted Factor Score",
          x = "Species",
          fill = "Factors"
        ) +
        theme_light() +
      ...
      final_fig <- final_fig |>
        layout(
          xaxis = list(
            range = c(0, max(for_plot$`Weighted Total Score`) + 0.5),
            tickmode = "linear",
            dtick = 1
          )
        )
  ...
  reactive_com_df <- reactive({
    ...
    reactive_com_df <- com_rev_data()[
      com_rev_data()$`Management Group` %in% input$com_species_selector,
    ]
    reactive_com_df <- com_rev_data() |>
  ...
  output$com_gt_table <- render_gt({
    ...
    com_table <- reactive_com_df() |>
      ...
      tab_header(
        ...
        subtitle = paste0(
          "Measured by total inflation adjusted ex-vessel revenue data ($1,000)
        between ",
          text_recent_five_years(),
          "."
        )
  ...
  create_base_plot <- function(df) {
    base_plot <- ggplot(
      ...
      geom_segment(
        aes(x = Species, xend = Species, y = Rank, yend = 65),
        color = "gray"
      ) +
  ...
  output$com_ranking <- renderPlotly({
    ...
    com_plot <- create_base_plot(com_rev_data()) +
      labs(
        ...
        x = "Species (in alphabetical order)",
        y = "Rank",
  ...
  reactive_rec_df <- reactive({
    ...
    reactive_rec_df <- rec_data()[
      rec_data()$`Management Group` %in% input$rec_species_selector,
    ]
    reactive_rec_df <- rec_data() |>
  ...
  output$rec_gt_table <- render_gt({
    ...
    rec_table <- reactive_rec_df() |>
      ...
      tab_header(
        ...
        subtitle = paste0(
          "Measured by total recreational catch between ",
          text_recent_five_years(),
          "."
        )
      ) |>
  ...
  reactive_tribal_df <- reactive({
    ...
    reactive_tribal_df <- tribal_data()[
      tribal_data()$`Management Group` %in% input$tribal_species_selector,
    ]
    reactive_tribal_df <- tribal_data() |>
  ...
  output$tribal_gt_table <- render_gt({
    ...
    tribal_table <- reactive_tribal_df() |>
      ...
      tab_header(
        ...
        subtitle = paste0(
          "Measured by total inflation adjusted ex-vessel revenue data for tribal landings
        between ",
          text_recent_five_years(),
          "."
        )
  ...
  reactive_cd_df <- reactive({
    ...
    reactive_cd_df <- const_dem_data()[
      const_dem_data()$`Management Group` %in% input$cd_species_selector,
    ]
    reactive_cd_df <- const_dem_data() |>
  ...
  output$cd_gt_table <- render_gt({
    ...
    if ("Projected ACL Attainment" %in% input$cd_columns) {
      cd_table <- cd_table |>
        ...
        tab_footnote(
          footnote = "Cells with red text indicate
  ...
  reactive_reb_df <- reactive({
    reactive_reb_df <- rebuilding_data()[
      rebuilding_data()$`Management Group` %in% input$reb_species_selector,
    ]
    reactive_reb_df <- rebuilding_data() |>
  ...
  output$reb_species_ranking <- renderPlotly({
    ...
    reb_plot <- ggplot(
      ...
      geom_segment(
        aes(x = Species, xend = Species, y = 0, yend = `Factor Score`),
        color = "gray"
      ) +
      geom_point(aes(color = `Management Group`), size = 3) +
      ylim(NA, 10) +
  ...
  reactive_ss_df <- reactive({
    reactive_ss_df <- stock_stat_data()[
      stock_stat_data()$`Management Group` %in% input$ss_species_selector,
    ]
    reactive_ss_df <- stock_stat_data() |>
  ...
  reactive_fm_df <- reactive({
    ...
    reactive_fm_df <- fish_mort_data()[
      fish_mort_data()$`Management Group` %in% input$fm_species_selector,
    ]
    reactive_fm_df <- fish_mort_data() |>
  ...
  output$fm_gt_table <- render_gt({
    ...
    fm_table <- reactive_fm_df() |>
      ...
      tab_header(
        ...
        subtitle = paste0(
          "Measured by average OFLs and average catch between ",
          text_recent_five_years(),
          "."
        )
      ) |>
    ...
    if (
      ...
      fm_table <- fm_table |>
        ...
        tab_footnote(
          footnote = "Cells with italic text indicate OFL contributions.",
  ...
  reactive_eco_df <- reactive({
    reactive_eco_df <- eco_data()[
      eco_data()$`Management Group` %in% input$eco_species_selector,
    ]
    reactive_eco_df <- eco_data() |>
  ...
  reactive_ni_df <- reactive({
    reactive_ni_df <- new_info_data()[
      new_info_data()$`Management Group` %in% input$ni_species_selector,
    ]
    reactive_ni_df <- new_info_data() |>
  ...
  reactive_af_df <- reactive({
    reactive_af_df <- assess_freq_data()[
      assess_freq_data()$`Management Group` %in% input$af_species_selector,
    ]
    reactive_af_df <- assess_freq_data() |>
